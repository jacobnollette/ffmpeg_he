		...
		if [ ! -f "$print_file" ]; then
			...
			if [ "$subtitles" = true ];
				then
					...
					ffmpeg -y -i "$original_item" -max_muxing_queue_size 4096 -vf scale="w=$width:trunc(ow/a/2)*2" -c:v "$videocodec" -pix_fmt yuv420p -b:v "$_the_videobitrate" -preset "$preset" -acodec copy -c:s copy -threads "$threads" -f "$filetype" -strict -2 "$print_file";

				else

					#	without subtitles
					echo "$original_item" > /tmp/ffmpeg_last
					ffmpeg -y -i "$original_item" -max_muxing_queue_size 4096 -vf scale="w=$width:trunc(ow/a/2)*2" -c:v "$videocodec" -pix_fmt yuv420p -b:v "$_the_videobitrate" -preset "$preset" -acodec copy -threads "$threads" -f "$filetype" -strict -2 "$print_file";
				fi
		fi;
